...
APIFY_ACTOR_ID = os.environ.get("APIFY_ACTOR_ID", "yZnhB5JewWf9xSmoM") 
...
# ---------- NEW PARSER LOGIC STARTS HERE ----------

# 1. Updated Prompt with Project Specific Instructions
...
# 2. Updated Examples with Multiple Projects
EXAMPLES = [
    lx.data.ExampleData(
...
def run_langextract(text: str) -> Dict[str, Any]:
    ...
    result = lx.extract(
        ...
        examples=EXAMPLES,
        model_id="gemini-2.5-flash", # Updated model ID per your new file
        api_key=GEMINI_API_KEY,
    )
    return result

# 3. Updated Project Parsing Helper
def parse_projects_from_extractions(extractions):
    ...
    for project_ext in project_extractions:
        ...
        # Basic parsing of project details
        project_data = {
            "name": "",
            "technologies": [],
            "description": "",
...
# 4. Updated Main Parse Function to match new extraction logic
def parse_resume_from_text(text: str) -> Dict[str, Any]:
    # call run_langextract (blocking) - intended to be run via asyncio.to_thread
    ...
    parsed = {
        ...
        "raw_project_count": len(grouped_data["projects"])  # Kept for debugging/stats
    }
